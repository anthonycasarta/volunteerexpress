...
class ReportPage extends StatelessWidget {
  ...
  Future<void> eventPDf(BuildContext context) async {
    ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
        content: Text('Events PDF created!'), duration: Duration(seconds: 2)));
    final reportsService =
        ReportsService(firestore: FirebaseFirestore.instance);
  ...
  Future<void> eventCSV(BuildContext context) async {
    ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
        content: Text('Events CSV created!'), duration: Duration(seconds: 2)));
    final reportsService =
        ReportsService(firestore: FirebaseFirestore.instance);
